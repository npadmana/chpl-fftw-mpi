...
type GlobalArrays = [rcDomain] LocalArrays;

var aList : GlobalArrays;
var localsum1, localsum2 : G_real;

// Initialize
coforall loc in Locales do on loc
{
  aList(1) = new LocalArrays();
  ref me = aList(1);
  var rng = makeRandomStream(real,seed=(me.rank+1)*1234);
  me.A = 0.0;
  rng.fillRandom(me.A[..,..,0.. #Ng]);
  me.B = me.A;
  localsum1(1) = + reduce me.A[..,..,0.. #Ng];
  localsum2(1) = + reduce me.A[..,..,0.. #Ng]**2;
}
...
coforall loc in Locales do on loc
{
  ...
  me.B *= invNg3;
  var Dx = me.D[..,..,0.. #Ng];
  localsum1(1) = max reduce abs(me.A[Dx]-me.B[Dx]);
}
